...
A Julia package for computing local and global 𝔸¹-Brouwer degrees
and studying the symmetric bilinear forms they produce, over ℚ,
finite fields of odd characteristic, ℝ, and ℂ.

The 𝔸¹-Brouwer degree of an endomorphism of affine space with
isolated zeros is a class in the Grothendieck–Witt ring GW(k) of
...
pointed rational functions ℙ¹ → ℙ¹ the degree lives in
the unstable group GWᵘ(k), which additionally records a
k^×-scalar.
...
- **𝔸¹-Brouwer degrees** — local and global, stable and unstable,
...
- **Finite étale algebras** — multiplication matrices, trace and norm, and the
  transfer map GW(L) → GW(k) along a finite étale extension.
...
The unstable degrees compute numerically over ℂ when given
...
|---|---|
| ℚ and 𝔽_q (odd q) | exact, via Oscar |
| ℝ | compute over ℚ and base-change the resulting Gram matrix (`Matrix{Float64}` represents forms over ℝ) |
| ℂ | stable degrees are rank-only and come from the exact computation; unstable degrees are computed numerically via HomotopyContinuation.jl |
...
- F. Morel, *𝔸¹-algebraic topology over a field*, Springer Lecture Notes in Mathematics, 2012.
- C. Cazanave, *Algebraic homotopy classes of rational functions*, Annales Scientifiques de l'École Normale Supérieure, 2012.
- J. L. Kass and K. Wickelgren, *The class of Eisenbud–Khimshiashvili–Levine is the local 𝔸¹-Brouwer degree*, Duke Mathematical Journal, 2019.
- T. Brazelton, S. McKean, and S. Pauli, *Bézoutians and the 𝔸¹-degree*, Algebra & Number Theory, 2023.
